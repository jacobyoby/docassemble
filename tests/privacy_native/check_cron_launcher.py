...
def check_stop(sig, task, pid_file):
    pid_file.write_text('')
    env = dict(os.environ, JOS81_CRON_PID_FILE=str(pid_file))
    process = subprocess.Popen(['bash', install.ROOT + '/webapp/run-cron.sh', task],
    ...
    try:
        ...
        # Signal only the root Bash PID, not its process group or Go child.
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='jos81-cron-stop-') as directory:
        ...
        for sig, task in ((signal.SIGTERM, 'cron_stop_term'), (signal.SIGINT, 'cron_stop_int')):
            ...
            check_stop(sig, task, pid_file)
    install.private_output_absent((Path(install.ROOT + '/log'), Path('/var/log')))
    print('TERM and INT through root cron launcher stop Go, Flask and descendants; only safe output remains', flush=True)
