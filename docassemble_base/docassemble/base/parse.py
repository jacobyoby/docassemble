        ...
        if debug:
            ...
            for question_type in ('question', 'help'):
                if question_type not in output:
                    continue
                phrase = to_text('<div>' + output[question_type] + '</div>')
...
class Question:
    ...
    def __init__(self, orig_data, caller, **kwargs):
        ...
        if 'submit' in data:
            self.submittext = TextObject(definitions + str(data['submit']), question=self)
        if 'check in' in data:
    ...
    def ask(self, user_dict, old_user_dict, the_x, iterators, sought, orig_sought, process_list_collect=True, test_for_objects=True):
        ...
        if hasattr(self, 'submittext') and self.submittext is not None:
            extras['submit text'] = self.submittext.text(user_dict)
        elif 'submit' in user_dict['_internal'] and user_dict['_internal']['submit'] is not None:
            extras['submit text'] = user_dict['_internal']['submit']
        elif self.language in self.interview.default_screen_parts and 'submit' in self.interview.default_screen_parts[self.language]:
            extras['submit text'] = self.interview.default_screen_parts[self.language]['submit'].text(user_dict)
        elif '*' in self.interview.default_screen_parts and 'submit' in self.interview.default_screen_parts['*']:
            extras['submit text'] = self.interview.default_screen_parts['*']['submit'].text(user_dict)
        elif 'submit' in the_default_titles:
            extras['submit text'] = the_default_titles['submit']
        for screen_part in ('footer', 'exit link', 'exit label', 'exit url', 'full', 'logo', 'short logo', 'title', 'subtitle', 'tab title', 'short title', 'title url', 'title url opens in other window', 'navigation bar html'):
        ...
        if len(self.attachments) > 0 or self.compute_attachment is not None:
            ...
            if hasattr(self, 'email_body'):
                extras['email_html'] = '<html><body>' + markdown_to_html(self.email_body.text(user_dict), status=this_thread.interview_status, question=self, external=True) + '</body></html>'
