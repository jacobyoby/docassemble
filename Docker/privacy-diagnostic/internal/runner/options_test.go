...
func TestApplicationProfilesRetainServiceShutdownWindows(t *testing.T) {
	for _, item := range []struct {
		name string
		wait time.Duration
	}{{"celery", 60 * time.Second}, {"celerysingle", 60 * time.Second}, {"websockets", 20 * time.Second}, {"mail", 20 * time.Second}, {"cron", 20 * time.Second}, {"initialize", 610 * time.Second}, {"maintenance", 30 * time.Second}} {
