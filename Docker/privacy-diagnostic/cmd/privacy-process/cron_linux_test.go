...
func TestFiniteCommandsReportOnlyAtExitWithoutMailInputOrRetrySemantics(t *testing.T) {
	for _, profile := range []string{"cron", "maintenance"} {
		t.Run(profile, func(t *testing.T) {
			...
			defer cancel()
			cmd := exec.CommandContext(ctx, self, "--component", profile, "--", "/bin/sh", "-c",
			...
			if _, err := json.Marshal(record); err != nil || record.Component != profile || record.Unclassified != 2 {
